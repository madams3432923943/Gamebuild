// Builds data/nfl-players.json and data/nfl-units.json from the raw nflverse
...
//
//   data/nfl-players.json  QB, RB, WR, TE - individuals, one row per player
//   data/nfl-units.json    OL, DL, LB, CB, S, ST - one row per team-era-group
//
...
const OUT_PLAYERS = join(here, "..", "data", "nfl-players.json");
const OUT_UNITS = join(here, "..", "data", "nfl-units.json");
...
// JSON, not a JS module. The browser was parsing 5.2MB of football as
// JAVASCRIPT to reach a draft board; JSON.parse is 4.7x faster on the
// identical payload. See data/README.md.
//
// `header` and `format` are gone with the module wrapper - JSON has nowhere to
// put a comment, and the per-row formatter existed only to control how the
// literal was laid out. The SORT stays: rows are still written grouped by team
// and season, so a regenerated file diffs the same way it always did.
function writeGenerated(path, rows, keyOf) {
  writeFileSync(path, JSON.stringify([...rows].sort((a, b) => keyOf(a).localeCompare(keyOf(b)))));
}

const squads = new Set(playerRows.map((p) => `${p.team}|${p.era}`));
const seasons = new Set(playerRows.map((p) => `${p.team}|${p.season}`));

writeGenerated(OUT_PLAYERS, playerRows, (p) => `${p.team} ${p.season}`);

writeGenerated(OUT_UNITS, unitRows, (u) => `${u.team} ${u.season}`);
