...
| `parity:dataset` | `data/nba-players.json` vs the `players` table |

That last one is not about code at all. The client normalizes every matchup
against `computeDatasetStats(PLAYERS)` from `data/nba-players.json`; the Edge Function
...
| Dataset (`data/nba-players.json` vs `players`) | **PASS** - verified out of band, see below |
...
| | `data/nba-players.json` | `players` table |
| --- | --- | --- |
| rows | 2542 | 2542 |
| squads | 163 | 163 |
...
| `verify:admin` | yes | Two things nobody would notice: a figure coming from a table read rather than a guarded RPC (the page would look identical and be open to every signed-in player), and a null metric drawn as `0%`. A 0% Day-1 retention nobody measured is not a blank - it is a finding somebody plans around. It also asserts the dashboard has not moved back to the site root, and that the local server it was replaced by binds loopback rather than every interface. |
| `verify:squads` | yes | Also covers the roster's six sort orders, including the one the tiering exists for: a 1-0 record is a 100% win rate and must not outrank 40-10. The comparators are pure, so they are checked directly - a browser is the wrong instrument for that question. |
