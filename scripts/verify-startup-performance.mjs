...
async function main() {
  ...
  try {
    ...
    checks.push({
      ...
      // instead: ~1.3MB measured, 1.5MB allowed. Raising this number back up is
      // how the regression would hide, so it should be argued for, not nudged.
      //
      // It was nudged once, to 1.6MB, when a four-kilobyte copy change to the
      // sign-in screen ran into 1,481 bytes of remaining headroom. That was the
      // wrong move and it has been reverted: football's engine, units, recap,
      // draftgrade and botdraft were being downloaded before a sport had been
      // chosen, and deferring them the way the datasets were deferred took
      // ~231KB off boot - sixty times what the copy change added. See
      // loadSimulation in js/sports/nfl/index.js.
      //
      // What is left is the app shell plus both sports' registry, constants and
      // tactics. If this gets tight again, the next lever is that pair of
      // constants files, not this number.
