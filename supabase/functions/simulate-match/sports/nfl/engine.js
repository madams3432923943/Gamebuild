...
function buildPlays(startYard, endYard, outcome, kind, scorerSlot, roster, rand) {
  ...
  for (let i = 0; i < order.length; i++) {
    ...
    let type;
    if (!slot.productive) type = slot.dead.type;
    ...
    const scoringPlay = last && (outcome === "touchdown") && scorerSlot;
    let carrier = null;
    let receiver = null;
    if (type === "run") {
      carrier = scoringPlay && kind === "rush" ? scorerSlot : pickBySlotWeight(usage.rushers, rand);
    } else if (type === "sack") {
      carrier = "QB";
    } else if (type === "incompletion") {
      receiver = null;
    } else {
      receiver = scoringPlay && kind === "rec" ? scorerSlot : pickBySlotWeight(usage.catchers, rand);
    }
